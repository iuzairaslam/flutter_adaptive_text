# flutter_adaptive_text — AI Context

## What this package does
`flutter_adaptive_text` automatically selects a legible text color (black, white, or any palette color) given a background color. It uses the WCAG 2.1 relative luminance formula with an optional APCA (WCAG 3.0) mode.

## Package structure
```
flutter_adaptive_text/
├── lib/
│ ├── flutter_adaptive_text.dart ← public barrel export
│ └── src/
...
- `getLuminance(Color)` — WCAG 2.1 relative luminance (0–1), delegates to `Color.computeLuminance()`
